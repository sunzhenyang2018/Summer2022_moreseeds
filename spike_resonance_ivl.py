...
ivl_paras = np.load("data/IVL_search/g_std_tau_for_rates_binned.npy")

fb_array = ivl_paras[:, -1]    # the last col is the rates
...
def resonance_exp_vivo(runtime, fi, ivl_paras):
    ...
    results = np.zeros((ivl_paras.shape[0], len(fi)))
    firing_rates = np.zeros((ivl_paras.shape[0], len(fi)))

    # set up ivl
    OU_in = h.Gfluct2(h.soma(0.5))
    OU_in.E_i = -87.1
    for k in range(ivl_paras.shape[0]):
        line = ivl_paras[k]
        OU_in.g_e0 = line[0]
        OU_in.g_i0 = line[1]
        OU_in.std_e = line[2]
    ...
    np.save('data/ext_baseline_ratio_vivo', results)
    np.save('data/ext_firing_rate_vivo', firing_rates)


def resonance_exp_vivo_inh(runtime, fi, ivl_paras):
    """This function finds baseline ratios and spike rate at each fi and ampb"""
    ...
    np.save('data/inh_baseline_ratio_vivo', results)
    np.save('data/inh_firing_rate_vivo', firing_rates)


def baseline_ratio_plotter(fi, fb, dot_height, bsr_path, savepath):
...
def fb_v_fr_plot(fi, fb, bsr_path, savepath):
    # bsr_path is the file that holds the baseline ratios
    bl_ratio = np.load(bsr_path)  # each row a diff fb, each col a diff fi
    baserates = fb
...
def fb_v_spikerate_fr_plot(spikerate_path, fb, bsr_path, savepath):
...
if __name__ == '__main__':
    p1 = mp.Process(target=resonance_exp_vivo, args=(10000, fi_array, ivl_paras))
    p2 = mp.Process(target=resonance_exp_vivo_inh, args=(10000, fi_array, ivl_paras))
    p1.start()
    p2.start()
    p1.join()
    p2.join()

    baseline_ratio_plotter(fi_array, fb_array, 65, "data/ext_baseline_ratio_vivo.npy",
                           "1_comp_plots/IVL/spike_res/ext_baseline_ratio_vivo.png")

    resonant_freq_histogram_plotter(fi_array, "data/ext_baseline_ratio_vivo.npy",
                                    "1_comp_plots/IVL/spike_res/ext_res_freq_vitro_hist.png")

    fb_v_fr_plot(fi_array, fb_array, "data/ext_baseline_ratio_vivo.npy",
                 '1_comp_plots/IVL/spike_res/ext_fb_v_fr_vivo.png')

    fb_v_spikerate_fr_plot("data/ext_firing_rate_vivo.npy", fb_array, "data/ext_baseline_ratio_vivo.npy",
                           "1_comp_plots/IVL/spike_res/ext_fb_v_spikerate_fr.png")

    baseline_ratio_plotter(fi_array, fb_array, 10.5, "data/inh_baseline_ratio_vivo.npy",
                           "1_comp_plots/IVL/spike_res/inh_baseline_ratio_vivo.png")
    resonant_freq_histogram_plotter(fi_array, "data/inh_baseline_ratio_vivo.npy",
                                    "1_comp_plots/IVL/spike_res/inh_res_freq_vitro_hist.png")
    fb_v_fr_plot(fi_array, fb_array, "data/inh_baseline_ratio_vivo.npy",
                 '1_comp_plots/IVL/spike_res/inh_fb_v_fr_vivo.png')
    fb_v_spikerate_fr_plot("data/inh_firing_rate_vivo.npy", fb_array, "data/inh_baseline_ratio_vivo.npy",
                           "1_comp_plots/IVL/spike_res/inh_fb_v_spikerate_fr.png")
